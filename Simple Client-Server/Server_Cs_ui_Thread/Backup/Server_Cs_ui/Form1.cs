                ...
                while (true)
                {
                    if (server.Pending())//проверяем, есть ли в очереди клиенты
                    {
                        ...
                        if (i > 0)
                        {
                            ...
                            byte[] answer_bytes = System.Text.Encoding.ASCII.GetBytes(answer_message);
                            // отправляем ответ
